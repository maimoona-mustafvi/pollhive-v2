      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="flex flex-wrap items-center justify-between gap-4 px-6 py-5 lg:px-8">
          ...
          <div className="flex items-center gap-3">
            {/* Replace the static search div with SearchBar component */}
            <SearchBar />
...
// SearchBar component added at the bottom of the file
function SearchBar() {
  ...
  function handleSearch(e: React.FormEvent) {
    e.preventDefault()
    if (query.trim()) {
      router.push(`/my-polls?q=${encodeURIComponent(query.trim())}`)
    }
  }
  ...
  return (
    <form
      ...
      className="hidden items-center gap-2 rounded-full bg-white px-3.5 py-2 text-sm text-muted-foreground shadow-sm md:flex"
    >
      ...
      <input
        ...
        className="w-32 bg-transparent outline-none placeholder:text-muted-foreground focus:w-48 transition-all duration-200"
      />
    </form>
  )
}